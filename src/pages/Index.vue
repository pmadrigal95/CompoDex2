<template>
    <div class="flex flex-col min-h-screen bg-background text-foreground">
            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8" v-motion :initial="{ opacity: 0 }"
                ...
                <div class="text-center max-w-3xl mx-auto mb-12">
                    <div v-motion :initial="{ opacity: 0, y: 20 }" :enter="{ opacity: 1, y: 0 }"
                        ...
                        <span class="px-3 py-1 text-xs font-medium rounded-full bg-primary/10 text-primary mb-4">
                            Vue Documentation Generator
                        </span>
                    </div>

                    <h1 class="text-4xl md:text-5xl font-bold tracking-tight mb-4" v-motion
                        :initial="{ opacity: 0, y: 20 }" :enter="{ opacity: 1, y: 0 }"
                        :transition="{ duration: 500, delay: 100 }">
                        Generate beautiful docs for your Vue components
                    </h1>

                    <p class="text-lg text-muted-foreground" v-motion :initial="{ opacity: 0, y: 20 }"
                        :enter="{ opacity: 1, y: 0 }" :transition="{ duration: 500, delay: 200 }">
                        Turn your Vue components into well-structured documentation with just a few clicks. Perfect
                        for teams and open-source projects.
                    </p>
                </div>
        ...
        <footer class="w-full py-6 px-4 border-t border-border mt-auto" v-motion :initial="{ opacity: 0 }"
